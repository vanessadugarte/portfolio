...
## Graphify como fuente de verdad del proyecto

- Carga y sigue la skill `graphify`. Usa exclusivamente sus consultas `query`, `path` y `explain` para descubrir qué archivos, símbolos, documentos, arquitectura, relaciones y convenciones son relevantes. Si el grafo no existe o necesita actualización, créalo o actualízalo mediante el flujo de Graphify antes de continuar.
- Graphify decide el alcance y las relaciones del trabajo. Una vez que Graphify haya identificado explícitamente un archivo o símbolo, puedes leer directamente solo ese archivo para obtener el texto literal vigente necesario para editarlo, preparar un parche o validar su comportamiento. Esta lectura operativa no sustituye a Graphify como mecanismo de descubrimiento y no requiere una autorización adicional del usuario.
- Limita la lectura directa a los archivos identificados y, cuando Graphify proporcione una ubicación, a la sección o rango cercano necesario. Si el texto leído menciona otra dependencia, archivo o convención que no está respaldado por el grafo, no lo sigas directamente: vuelve a `query`, `path` o `explain` y deja que Graphify confirme su relevancia antes de leerlo.
- No busques ni enumeres el proyecto mediante mecanismos alternativos. Quedan prohibidos como vía de descubrimiento `grep`, `rg`, `git grep`, `find`, `fd`, `ls`, `tree`, globbing, scripts de búsqueda, búsquedas globales del IDE y búsquedas de contenido o historial con Git. `cat`, `sed`, `awk`, `head`, `tail` o equivalentes solo pueden usarse para leer un archivo que Graphify ya identificó, nunca para localizar candidatos o ampliar el alcance.
- Las instrucciones de repositorio entregadas por el entorno pueden leerse directamente porque son autoridad operativa. Para cualquier documento adicional de versionamiento, contribución, pruebas o PR, deja que Graphify lo identifique antes de leer su texto completo.
- Si Graphify no logra identificar el archivo, símbolo o relación necesarios después de actualizar el grafo y consultar con vocabulario adecuado, detente, explica qué información falta y pide autorización explícita antes de usar otra vía de descubrimiento. No pidas permiso únicamente porque `query`, `path` o `explain` no muestran el cuerpo completo de un archivo que el propio grafo ya identificó.
- Esta restricción no impide consultar el issue y el PR en GitHub ni ejecutar operaciones que no exploran contenido: estado, remotos y ramas de Git; `fetch`, cambio de rama, staging, commits y push; edición de archivos ya determinados por Graphify; y pruebas, lint o build. El diff queda limitado a revisar los cambios del issue, no a descubrir código relacionado.
